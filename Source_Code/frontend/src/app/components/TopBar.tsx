            ...
            {showAlerts && (
              ...
              >
                <div className={styles.dropdownHeader}>
                  <span style={{ fontWeight: 700, fontSize: '0.85rem' }}>Active Alerts</span>
                  <motion.button whileHover={{ scale: 1.1 }} onClick={() => setShowAlerts(false)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.4)' }}
                  ><X size={14} /></motion.button>
                </div>
                {[
                  { station: 'Trichy Upper', status: 'danger',  msg: 'Level at 79% — HIGH risk', time: '2m ago' },
                  { station: 'Karur',        status: 'warning', msg: 'Rising trend detected',    time: '14m ago' },
                  { station: 'Mettur Dam',   status: 'alert',   msg: 'Reservoir at 87%',          time: '1h ago' },
                ].map(a => (
                  <div key={a.station} className={styles.dropdownItem}>
                    <span className={`status-dot status-${a.status}`} style={{ width: 6, height: 6, flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{a.station}</div>
                      <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.45)' }}>{a.msg}</div>
                    </div>
                    <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.28)', whiteSpace: 'nowrap' }}>{a.time}</span>
                  </div>
                ))}
