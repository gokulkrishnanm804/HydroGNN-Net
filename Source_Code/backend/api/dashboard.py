...
def get_dashboard_summary(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    # 1. Resolve latest telemetry timestamp.
    #    CRITICAL (Issue #1 fix): clamp to ts <= utcnow() so real-time rows
    #    (e.g. 2026-07-13) always win over old simulation rows (e.g. 2026-11-01).
    #    Without this, ORDER BY DESC returns the highest timestamp, which could
    #    be a future simulated value that masks today's live ingestion.
